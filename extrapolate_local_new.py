...
# matlab/extrapolate_local_new.m
...
# 04/12/2013    DB  RBut the box value to the closest non-NaN value
    ...
    if nargin < 5 or isempty(n_pixels_boundary):
        n_pixels_boundary=5
# matlab/extrapolate_local_new.m:39
    
    if nargin < 6 or isempty(R):
        R=0.2
# matlab/extrapolate_local_new.m:42
        # averaged to find the datavalue at the grid point location.
    
    flag_control_fig=0
# matlab/extrapolate_local_new.m:45
    
    # converting the resolutions to km
    x_res=x_res / 1000
# matlab/extrapolate_local_new.m:49
    
    y_res=y_res / 1000
# matlab/extrapolate_local_new.m:50
    
    # making sure the inputted data is given as a double
    xy_local=double(xy_local)
# matlab/extrapolate_local_new.m:53
    z=double(z)
# matlab/extrapolate_local_new.m:54
    # computed grid extremes
    xy_min=min(xy_local)
# matlab/extrapolate_local_new.m:57
    xy_max=max(xy_local)
# matlab/extrapolate_local_new.m:58
...
# matlab/extrapolate_local_new.m:62
    y_res_area=dot(n_pixels_boundary,y_res)
# matlab/extrapolate_local_new.m:63
...
# matlab/extrapolate_local_new.m:81
    n_datasets=size(z,2)
# matlab/extrapolate_local_new.m:82
    z_grid=NaN(cat(n_grid_poits,n_datasets))
# matlab/extrapolate_local_new.m:83
    # interferograms which contain only NaN vlaues
    ix_ifg=find(sum(isnan(z)) == n_grid_poits)
# matlab/extrapolate_local_new.m:86
...
# matlab/extrapolate_local_new.m:93
    row_ix=unique(row_ix)
# matlab/extrapolate_local_new.m:94
    xy_local_temp=copy(xy_local)
# matlab/extrapolate_local_new.m:95
    z_temp=copy(z)
# matlab/extrapolate_local_new.m:96
    xy_local_temp[row_ix,:]=[]
# matlab/extrapolate_local_new.m:97
    z_temp[row_ix,:]=[]
# matlab/extrapolate_local_new.m:98
    if R == inf:
        # take the average of these points to set as new grid value
        z_grid=repmat(nanmean(z_temp,1),n_grid_poits,1)
# matlab/extrapolate_local_new.m:102
    else:
...
# matlab/extrapolate_local_new.m:107
            min_distance=min(Distance)
# matlab/extrapolate_local_new.m:110
            ix=find(Distance - (min_distance + R) <= 0)
# matlab/extrapolate_local_new.m:113
            clear('Distance')
            # take the average of these points to set as new grid value
            z_grid[k,:]=nanmean(z_temp[ix,:],1)
# matlab/extrapolate_local_new.m:117
            if flag_control_fig == 1:
                if k == 1:
                    h1=figure('name','Extrapolated data')
# matlab/extrapolate_local_new.m:123
                hold('off')
...
# matlab/extrapolate_local_new.m:152
    z_box=matlabarray(cat([z],[z_grid]))
# matlab/extrapolate_local_new.m:153
    if flag_control_fig == 1:
