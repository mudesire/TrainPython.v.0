...
# 07/07/2016    DB      Redefine hydrostatic delay to be based on surface pressure.
    ...
    save_complete=0
# Trainmatlab.v.0/aps_wrf_SAR.m:58
    
    ## Constants
# Parameters from Jolivet et al 2011, GRL
    Rd=287.05
# Trainmatlab.v.0/aps_wrf_SAR.m:63
    
    Rv=461.495
# Trainmatlab.v.0/aps_wrf_SAR.m:64
    
    k1=0.776
# Trainmatlab.v.0/aps_wrf_SAR.m:65
    
    k2=0.716
# Trainmatlab.v.0/aps_wrf_SAR.m:66
    
    k3=3750.0
# Trainmatlab.v.0/aps_wrf_SAR.m:67
    
    ## Defaults
    zref=15000
# Trainmatlab.v.0/aps_wrf_SAR.m:71
    
    zincr=15
# Trainmatlab.v.0/aps_wrf_SAR.m:72
    
    vertres=5
# Trainmatlab.v.0/aps_wrf_SAR.m:73
    
    ## get the number of domains
    repeat=1
# Trainmatlab.v.0/aps_wrf_SAR.m:77
    while repeat == 1:

        action_flag=str2num(input_('How many domains do you have including the parent? ','s'))
# Trainmatlab.v.0/aps_wrf_SAR.m:79
        if isnumeric(action_flag):
            n_domains=copy(action_flag)
# Trainmatlab.v.0/aps_wrf_SAR.m:81
            repeat=0
# Trainmatlab.v.0/aps_wrf_SAR.m:82

    
    ## getting the variables from the parms_aps file
    stamps_processed=getparm_aps('stamps_processed',1)
# Trainmatlab.v.0/aps_wrf_SAR.m:88
    ll_matfile=getparm_aps('ll_matfile',1)
# Trainmatlab.v.0/aps_wrf_SAR.m:89
    wrf_datapath=getparm_aps('wrf_datapath',1)
# Trainmatlab.v.0/aps_wrf_SAR.m:90
    datestructure='yyyymmdd'
# Trainmatlab.v.0/aps_wrf_SAR.m:91
    
    UTC_sat=getparm_aps('UTC_sat',1)
# Trainmatlab.v.0/aps_wrf_SAR.m:92
    # loading the data
    if strcmp(stamps_processed,'y'):
        ps=load(ll_matfile)
# Trainmatlab.v.0/aps_wrf_SAR.m:96
        dates=ps.day
# Trainmatlab.v.0/aps_wrf_SAR.m:97
        load('psver')
        fprintf('Stamps processed structure \\n')
    else:
        psver=2
# Trainmatlab.v.0/aps_wrf_SAR.m:101
        ifgday_matfile=getparm_aps('ifgday_matfile',1)
# Trainmatlab.v.0/aps_wrf_SAR.m:102
        ifgs_dates=load(ifgday_matfile)
# Trainmatlab.v.0/aps_wrf_SAR.m:103
        ifgs_dates=ifgs_dates.ifgday
# Trainmatlab.v.0/aps_wrf_SAR.m:104
        dates=reshape(ifgs_dates,[],1)
# Trainmatlab.v.0/aps_wrf_SAR.m:105
        dates=unique(dates)
# Trainmatlab.v.0/aps_wrf_SAR.m:106
        dates=datenum(num2str(dates),'yyyymmdd')
# Trainmatlab.v.0/aps_wrf_SAR.m:107
    
    # getting the dates
    n_dates=length(dates)
# Trainmatlab.v.0/aps_wrf_SAR.m:112
    ## Compute and resample DEM
    dem,xmin,xmax,ymin,ymax,smpres,nncols,nnrows=get_DEM
# Trainmatlab.v.0/aps_wrf_SAR.m:115
...
# Should be  larger than the region to which the delay is computed
    lonmin=floor(xmin) - 1
# Trainmatlab.v.0/aps_wrf_SAR.m:119
    lonmax=ceil(xmax) + 1
# Trainmatlab.v.0/aps_wrf_SAR.m:120
    latmin=floor(ymin) - 1
# Trainmatlab.v.0/aps_wrf_SAR.m:121
    latmax=ceil(ymax) + 1
# Trainmatlab.v.0/aps_wrf_SAR.m:122
...
# Trainmatlab.v.0/aps_wrf_SAR.m:127
...
# Trainmatlab.v.0/aps_wrf_SAR.m:217
            ix_vert=find(sum(sum(isnan(Temp))) == n_grid_points)
# Trainmatlab.v.0/aps_wrf_SAR.m:218
            if isempty(ix_vert):
                ix_vert=size(Temp,3)
# Trainmatlab.v.0/aps_wrf_SAR.m:220
            longrid_vector=reshape(longrid[:,:,1],[],1)
# Trainmatlab.v.0/aps_wrf_SAR.m:222
            latgrid_vector=reshape(latgrid[:,:,1],[],1)
# Trainmatlab.v.0/aps_wrf_SAR.m:223
            for kkk in arange(1,ix_vert[1] - 1).reshape(-1):
                ix=find(isnan(Temp[:,:,kkk]) == 1)
# Trainmatlab.v.0/aps_wrf_SAR.m:227
                if logical_not(isempty(ix)):
                    Temp_temp=reshape(Temp[:,:,kkk],[],1)
# Trainmatlab.v.0/aps_wrf_SAR.m:229
                    Hum_temp=reshape(Hum[:,:,kkk],[],1)
# Trainmatlab.v.0/aps_wrf_SAR.m:230
                    H_temp=reshape(H[:,:,kkk],[],1)
# Trainmatlab.v.0/aps_wrf_SAR.m:231
                    longrid_vector_temp=copy(longrid_vector)
# Trainmatlab.v.0/aps_wrf_SAR.m:233
                    latgrid_vector_temp=copy(latgrid_vector)
# Trainmatlab.v.0/aps_wrf_SAR.m:234
                    longrid_vector_needed=longrid_vector[ix]
# Trainmatlab.v.0/aps_wrf_SAR.m:237
                    latgrid_vector_needed=latgrid_vector[ix]
# Trainmatlab.v.0/aps_wrf_SAR.m:238
                    Temp_temp[ix]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:241
                    Hum_temp[ix]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:242
                    H_temp[ix]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:243
                    longrid_vector_temp[ix]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:245
                    latgrid_vector_temp[ix]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:246
...
# Trainmatlab.v.0/aps_wrf_SAR.m:250
...
# Trainmatlab.v.0/aps_wrf_SAR.m:251
...
# Trainmatlab.v.0/aps_wrf_SAR.m:252
                    Temp_new=Temp[:,:,kkk]
# Trainmatlab.v.0/aps_wrf_SAR.m:253
                    Hum_new=Hum[:,:,kkk]
# Trainmatlab.v.0/aps_wrf_SAR.m:254
                    H_new=H[:,:,kkk]
# Trainmatlab.v.0/aps_wrf_SAR.m:255
                    Temp_new[ix]=Temp_vector_needed
# Trainmatlab.v.0/aps_wrf_SAR.m:258
                    Hum_new[ix]=Hum_vector_needed
# Trainmatlab.v.0/aps_wrf_SAR.m:259
                    H_new[ix]=H_vector_needed
# Trainmatlab.v.0/aps_wrf_SAR.m:260
                    Temp[:,:,kkk]=Temp_new
# Trainmatlab.v.0/aps_wrf_SAR.m:263
                    Hum[:,:,kkk]=Hum_new
# Trainmatlab.v.0/aps_wrf_SAR.m:264
                    H[:,:,kkk]=H_new
# Trainmatlab.v.0/aps_wrf_SAR.m:265
...
# Trainmatlab.v.0/aps_wrf_SAR.m:272
            lat_res=dot(abs(latlist_temp[ceil(dot(length(latlist_temp),0.95))]),1.5)
# Trainmatlab.v.0/aps_wrf_SAR.m:273
            lonlist_temp=diff(lonlist)
# Trainmatlab.v.0/aps_wrf_SAR.m:275
            lon_res=dot(max(abs(lonlist_temp)),1.5)
# Trainmatlab.v.0/aps_wrf_SAR.m:276
...
# Trainmatlab.v.0/aps_wrf_SAR.m:280
            xlist=xlist[ix]
# Trainmatlab.v.0/aps_wrf_SAR.m:281
            ylist=ylist[ix]
# Trainmatlab.v.0/aps_wrf_SAR.m:282
            latlist=latlist[ix]
# Trainmatlab.v.0/aps_wrf_SAR.m:283
            lonlist=lonlist[ix]
# Trainmatlab.v.0/aps_wrf_SAR.m:284
            latlist=double(latlist)
# Trainmatlab.v.0/aps_wrf_SAR.m:285
            lonlist=double(lonlist)
# Trainmatlab.v.0/aps_wrf_SAR.m:286
            numy=length(unique(latlist))
# Trainmatlab.v.0/aps_wrf_SAR.m:288
            numx=length(unique(lonlist))
# Trainmatlab.v.0/aps_wrf_SAR.m:289
            ulatlist=unique(latlist)
# Trainmatlab.v.0/aps_wrf_SAR.m:290
            ulonlist=unique(lonlist)
# Trainmatlab.v.0/aps_wrf_SAR.m:291
            uxlist=unique(xlist)
# Trainmatlab.v.0/aps_wrf_SAR.m:292
            uylist=unique(ylist)
# Trainmatlab.v.0/aps_wrf_SAR.m:293
            if save_complete == 0:
                # saving the information for support plotting
                wrf.wrf_lonlat = copy(cat(lonlist,latlist))
# Trainmatlab.v.0/aps_wrf_SAR.m:301
                wrf.region = copy(cat(cat(xmin,xmin,xmax,xmax,xmin).T,cat(ymin,ymax,ymax,ymin,ymin).T))
# Trainmatlab.v.0/aps_wrf_SAR.m:302
                deminfo.xmin = copy(xmin)
# Trainmatlab.v.0/aps_wrf_SAR.m:303
                deminfo.xmax = copy(xmax)
# Trainmatlab.v.0/aps_wrf_SAR.m:304
                deminfo.ymax = copy(ymax)
# Trainmatlab.v.0/aps_wrf_SAR.m:305
                deminfo.ymin = copy(ymin)
# Trainmatlab.v.0/aps_wrf_SAR.m:306
                deminfo.dem = copy(dem)
# Trainmatlab.v.0/aps_wrf_SAR.m:307
                wrf.deminfo = copy(deminfo)
# Trainmatlab.v.0/aps_wrf_SAR.m:308
                clear('deminfo')
                ...
                save_complete=1
# Trainmatlab.v.0/aps_wrf_SAR.m:316
...
# Trainmatlab.v.0/aps_wrf_SAR.m:326
            svpi=multiply(6.1121,exp((multiply(22.587,(Temp - 273.16))) / (273.86 + Temp - 273.16)))
# Trainmatlab.v.0/aps_wrf_SAR.m:327
            tempbound1=273.16
# Trainmatlab.v.0/aps_wrf_SAR.m:328
            tempbound2=250.16
# Trainmatlab.v.0/aps_wrf_SAR.m:329
            svp=copy(svpw)
# Trainmatlab.v.0/aps_wrf_SAR.m:332
            wgt=(Temp - tempbound2) / (tempbound1 - tempbound2)
# Trainmatlab.v.0/aps_wrf_SAR.m:334
            svp=svpi + multiply((svpw - svpi),wgt ** 2)
# Trainmatlab.v.0/aps_wrf_SAR.m:335
            ix_bound1=find(Temp > tempbound1)
# Trainmatlab.v.0/aps_wrf_SAR.m:336
            svp[ix_bound1]=svpw[ix_bound1]
# Trainmatlab.v.0/aps_wrf_SAR.m:337
            ix_bound2=find(Temp < tempbound2)
# Trainmatlab.v.0/aps_wrf_SAR.m:338
            svp[ix_bound2]=svpi[ix_bound2]
# Trainmatlab.v.0/aps_wrf_SAR.m:339
            e=dot(Hum / 100.0,svp)
# Trainmatlab.v.0/aps_wrf_SAR.m:341
            # stated (incorrectly) in Jolivet et al. 2011.
            P=copy(Pressure)
# Trainmatlab.v.0/aps_wrf_SAR.m:344
            g0=9.80665
# Trainmatlab.v.0/aps_wrf_SAR.m:348
            g=multiply(9.80616,(1 - multiply(0.002637,cosd(dot(2.0,latgrid))) + multiply(5.9e-06,(cosd(dot(2.0,latgrid))) ** 2)))
# Trainmatlab.v.0/aps_wrf_SAR.m:351
            Rmax=6378137
# Trainmatlab.v.0/aps_wrf_SAR.m:353
            Rmin=6356752
# Trainmatlab.v.0/aps_wrf_SAR.m:354
            Re=sqrt(1.0 / (((cosd(latgrid) ** 2) / Rmax ** 2) + ((sind(latgrid) ** 2) / Rmin ** 2)))
# Trainmatlab.v.0/aps_wrf_SAR.m:355
            Z=(multiply(H,Re)) / (multiply(g / g0,Re) - H)
# Trainmatlab.v.0/aps_wrf_SAR.m:359
            midx=round(mean(uxlist))
# Trainmatlab.v.0/aps_wrf_SAR.m:362
            midy=round(mean(uylist))
# Trainmatlab.v.0/aps_wrf_SAR.m:363
            glocal=g[midy,midx,1]
# Trainmatlab.v.0/aps_wrf_SAR.m:364
            Rlocal=Re[midy,midx,1]
# Trainmatlab.v.0/aps_wrf_SAR.m:365
            cdslices=maxdem / vertres + 1
# Trainmatlab.v.0/aps_wrf_SAR.m:370
            cdstack=zeros(size(lonlist,1),cdslices)
# Trainmatlab.v.0/aps_wrf_SAR.m:371
            cdstack_dry=zeros(size(lonlist,1),cdslices)
# Trainmatlab.v.0/aps_wrf_SAR.m:372
            cdstack_wet=zeros(size(lonlist,1),cdslices)
# Trainmatlab.v.0/aps_wrf_SAR.m:373
            XI=(arange(0,zref,zincr)).T
# Trainmatlab.v.0/aps_wrf_SAR.m:375
            gh=multiply(glocal,(Rlocal / (Rlocal + XI)) ** 2)
# Trainmatlab.v.0/aps_wrf_SAR.m:376
        ...
        # then integrate using trapz to estimate delay as function of height
            for m in arange(1,size(lonlist,1)).reshape(-1):
                xn=xlist[m]
# Trainmatlab.v.0/aps_wrf_SAR.m:381
                yn=ylist[m]
# Trainmatlab.v.0/aps_wrf_SAR.m:382
                X=double(squeeze(Z[yn,xn,:]))
# Trainmatlab.v.0/aps_wrf_SAR.m:385
                YP=double(squeeze(P[yn,xn,:]))
# Trainmatlab.v.0/aps_wrf_SAR.m:386
                Ye=double(squeeze(e[yn,xn,:]))
# Trainmatlab.v.0/aps_wrf_SAR.m:387
                YT=double(squeeze(Temp[yn,xn,:]))
# Trainmatlab.v.0/aps_wrf_SAR.m:388
                YT[isnan(Ye)]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:391
                YP[isnan(Ye)]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:392
                X[isnan(Ye)]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:393
                Ye[isnan(Ye)]=[]
# Trainmatlab.v.0/aps_wrf_SAR.m:394
                YeI=dot(interp1(X,Ye,XI,'spline'),100)
# Trainmatlab.v.0/aps_wrf_SAR.m:396
                YPI=dot(interp1(X,YP,XI,'spline'),100)
# Trainmatlab.v.0/aps_wrf_SAR.m:397
                YTI=interp1(X,YT,XI,'spline')
# Trainmatlab.v.0/aps_wrf_SAR.m:398
                tmp1=(multiply((k2 - (dot(Rd,k1) / Rv)),YeI) / YTI + multiply(k3,YeI) / (YTI ** 2))
# Trainmatlab.v.0/aps_wrf_SAR.m:401
                Lw=dot(multiply((10 ** - 6),- 1),flipud(cumtrapz(flipud(XI),flipud(tmp1))))
# Trainmatlab.v.0/aps_wrf_SAR.m:402
                tmp2=multiply(k1,YPI) / YTI
# Trainmatlab.v.0/aps_wrf_SAR.m:404
                gm=copy(glocal)
# Trainmatlab.v.0/aps_wrf_SAR.m:406
                Ld=multiply((10 ** - 6),(multiply((dot(k1,Rd) / gm),(YPI - YPI[zref / zincr + 1]))))
# Trainmatlab.v.0/aps_wrf_SAR.m:407
...
# Trainmatlab.v.0/aps_wrf_SAR.m:411
                LdI=interp1(XI,Ld,cdI,'spline')
# Trainmatlab.v.0/aps_wrf_SAR.m:412
                LwI=interp1(XI,Lw,cdI,'spline')
# Trainmatlab.v.0/aps_wrf_SAR.m:413
                cdstack_dry[m,:]=LdI
# Trainmatlab.v.0/aps_wrf_SAR.m:416
                cdstack_wet[m,:]=LwI
# Trainmatlab.v.0/aps_wrf_SAR.m:417
            # Interpolate each cdstack layer onto a grid given by the DEM extents
        # in UTM m.
            xsmpres=(xmax - xmin) / nncols
# Trainmatlab.v.0/aps_wrf_SAR.m:422
            ysmpres=(ymax - ymin) / nnrows
# Trainmatlab.v.0/aps_wrf_SAR.m:423
