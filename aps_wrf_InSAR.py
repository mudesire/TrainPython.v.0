...
# Trainmatlab.v.0/aps_wrf_InSAR.m
...
#                   variable definition of dates, definition of lonlat, look angle.
    ...
    wetoutfile='_ZWD.xyz'
# Trainmatlab.v.0/aps_wrf_InSAR.m:44
    hydroutfile='_ZHD.xyz'
# Trainmatlab.v.0/aps_wrf_InSAR.m:45
    # getting the variables from the parms_aps file
    stamps_processed=getparm_aps('stamps_processed')
# Trainmatlab.v.0/aps_wrf_InSAR.m:48
    ll_matfile=getparm_aps('ll_matfile')
# Trainmatlab.v.0/aps_wrf_InSAR.m:49
    wrf_datapath=getparm_aps('wrf_datapath')
# Trainmatlab.v.0/aps_wrf_InSAR.m:50
    lambda_=dot(getparm_aps('lambda'),100)
# Trainmatlab.v.0/aps_wrf_InSAR.m:51
    
    datestructure='yyyymmdd'
# Trainmatlab.v.0/aps_wrf_InSAR.m:52
    
    # getting the dropped ifgs
    drop_ifg_index=getparm('drop_ifg_index')
# Trainmatlab.v.0/aps_wrf_InSAR.m:54
    # loading the data
    if strcmp(stamps_processed,'y'):
        fprintf('Stamps processed structure \\n')
        ps=load(ll_matfile)
# Trainmatlab.v.0/aps_wrf_InSAR.m:59
        load('psver')
        dates=ps.day
# Trainmatlab.v.0/aps_wrf_InSAR.m:61
        lonlat=ps.lonlat
# Trainmatlab.v.0/aps_wrf_InSAR.m:62
        look_angle=getparm_aps('look_angle')
# Trainmatlab.v.0/aps_wrf_InSAR.m:63
        if ischar(look_angle) == 1:
            look_angle=load(look_angle)
# Trainmatlab.v.0/aps_wrf_InSAR.m:65
            look_angle=look_angle.la
# Trainmatlab.v.0/aps_wrf_InSAR.m:66
        # getting the parms file list from stamps to see the final ifg list
        if strcmp(getparm('small_baseline_flag'),'y'):
            sb_flag=1
# Trainmatlab.v.0/aps_wrf_InSAR.m:71
        else:
            sb_flag=0
# Trainmatlab.v.0/aps_wrf_InSAR.m:73
        n_ifg=ps.n_ifg
# Trainmatlab.v.0/aps_wrf_InSAR.m:76
        if sb_flag == 1:
            # for SB
            ifg_number=cat(arange(1,n_ifg)).T
# Trainmatlab.v.0/aps_wrf_InSAR.m:80
            ifgday_ix=ps.ifgday_ix
# Trainmatlab.v.0/aps_wrf_InSAR.m:81
            ifgday_ix[drop_ifg_index,:]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:83
            ifg_number[drop_ifg_index]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:84
            ifgs_ix=matlabarray(cat(ifgday_ix,ifg_number))
# Trainmatlab.v.0/aps_wrf_InSAR.m:87
        else:
            # slightly different for PS.
            date_slave_ix=cat(arange(1,n_ifg)).T
# Trainmatlab.v.0/aps_wrf_InSAR.m:90
            ifg_number=cat(arange(1,n_ifg)).T
# Trainmatlab.v.0/aps_wrf_InSAR.m:91
            date_slave_ix[drop_ifg_index]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:94
            ifg_number[drop_ifg_index]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:95
            date_master_ix=repmat(ps.master_ix,size(date_slave_ix,1),1)
# Trainmatlab.v.0/aps_wrf_InSAR.m:98
            ifgs_ix=matlabarray(cat(date_master_ix,date_slave_ix,ifg_number))
# Trainmatlab.v.0/aps_wrf_InSAR.m:101
    else:
        psver=2
# Trainmatlab.v.0/aps_wrf_InSAR.m:106
        sb_flag=0
# Trainmatlab.v.0/aps_wrf_InSAR.m:109
        lonlat=load(ll_matfile)
# Trainmatlab.v.0/aps_wrf_InSAR.m:111
        lonlat=lonlat.lonlat
# Trainmatlab.v.0/aps_wrf_InSAR.m:112
        look_angle=getparm_aps('look_angle')
# Trainmatlab.v.0/aps_wrf_InSAR.m:113
        if ischar(look_angle) == 1:
            look_angle=load(look_angle)
# Trainmatlab.v.0/aps_wrf_InSAR.m:115
            look_angle=look_angle.la
# Trainmatlab.v.0/aps_wrf_InSAR.m:116
        # getting the dates in jullian format
        ifgday_matfile=getparm_aps('ifgday_matfile')
# Trainmatlab.v.0/aps_wrf_InSAR.m:120
        ifgs_dates=load(ifgday_matfile)
# Trainmatlab.v.0/aps_wrf_InSAR.m:121
        ifgs_dates=ifgs_dates.ifgday
# Trainmatlab.v.0/aps_wrf_InSAR.m:122
        dates=reshape(ifgs_dates,[],1)
# Trainmatlab.v.0/aps_wrf_InSAR.m:123
        dates=unique(dates)
# Trainmatlab.v.0/aps_wrf_InSAR.m:124
        dates=datenum(num2str(dates),'yyyymmdd')
# Trainmatlab.v.0/aps_wrf_InSAR.m:125
        dates=sort(dates)
# Trainmatlab.v.0/aps_wrf_InSAR.m:126
        # getting the ix position for the master and slave dates with respect
    # to the times
        date_master=datenum(num2str(ifgs_dates[:,1]),'yyyymmdd')
# Trainmatlab.v.0/aps_wrf_InSAR.m:130
        date_slave=datenum(num2str(ifgs_dates[:,2]),'yyyymmdd')
# Trainmatlab.v.0/aps_wrf_InSAR.m:131
        ifg_number=cat(arange(1,size(date_master,1))).T
# Trainmatlab.v.0/aps_wrf_InSAR.m:132
        date_slave[drop_ifg_index]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:135
        date_master[drop_ifg_index]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:136
        ifg_number[drop_ifg_index]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:137
        for k in arange(1,size(date_master,1)).reshape(-1):
            date_master_ix[k,1]=find(date_master[k,1] == dates)
# Trainmatlab.v.0/aps_wrf_InSAR.m:139
            date_slave_ix[k,1]=find(date_slave[k,1] == dates)
# Trainmatlab.v.0/aps_wrf_InSAR.m:140
        # ix interferograms
        ifgs_ix=matlabarray(cat(date_master_ix,date_slave_ix,ifg_number))
# Trainmatlab.v.0/aps_wrf_InSAR.m:144
        n_ifg=size(ifgs_dates,1)
# Trainmatlab.v.0/aps_wrf_InSAR.m:147
        ps.day = copy(dates)
# Trainmatlab.v.0/aps_wrf_InSAR.m:150
    
    n_dates=length(dates)
# Trainmatlab.v.0/aps_wrf_InSAR.m:155
    InSAR_datapath=matlabarray(cat('.',filesep))
# Trainmatlab.v.0/aps_wrf_InSAR.m:156
    apsname=matlabarray(cat(InSAR_datapath,filesep,'tca',num2str(psver),'.mat'))
# Trainmatlab.v.0/aps_wrf_InSAR.m:157
    apssbname=matlabarray(cat(InSAR_datapath,filesep,'tca_sb',num2str(psver),'.mat'))
# Trainmatlab.v.0/aps_wrf_InSAR.m:158
    ## loading the WRF-I data
# initialisation
    d_wrf_wet=NaN(cat(size(lonlat,1),n_dates))
# Trainmatlab.v.0/aps_wrf_InSAR.m:165
    
    d_wrf_dry=NaN(cat(size(lonlat,1),n_dates))
# Trainmatlab.v.0/aps_wrf_InSAR.m:166
    
    ix_no_wrf=matlabarray([])
# Trainmatlab.v.0/aps_wrf_InSAR.m:169
    counter=0
# Trainmatlab.v.0/aps_wrf_InSAR.m:170
...
# Trainmatlab.v.0/aps_wrf_InSAR.m:229
    ph_SAR_wrf_dry=multiply(dot(- 4,pi) / lambda_,d_wrf_dry)
# Trainmatlab.v.0/aps_wrf_InSAR.m:230
    ph_SAR_wrf_wet=multiply(dot(- 4,pi) / lambda_,d_wrf_wet)
# Trainmatlab.v.0/aps_wrf_InSAR.m:231
...
# Trainmatlab.v.0/aps_wrf_InSAR.m:241
            ifgs_ix[ix_ifg_reject,:]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:242
            ix_ifg_reject=find(ix_no_wrf[k] == ifgs_ix[:,1])
# Trainmatlab.v.0/aps_wrf_InSAR.m:244
            ifgs_ix[ix_ifg_reject,:]=[]
# Trainmatlab.v.0/aps_wrf_InSAR.m:245
            clear('ix_ifg_reject')
    
    # computing the interferometric delay for each remaining interferogram
    n_ifg_wrf=size(ifgs_ix,1)
# Trainmatlab.v.0/aps_wrf_InSAR.m:251
    if isempty(ifgs_ix):
