...
# line segment pairs.
    ...
    if 2 == nargin:
        robust=copy(true)
# matlab/intersections.m:96
        x2=copy(x1)
# matlab/intersections.m:97
        y2=copy(y1)
# matlab/intersections.m:98
        self_intersect=copy(true)
# matlab/intersections.m:99
    else:
        if 3 == nargin:
            robust=copy(x2)
# matlab/intersections.m:101
            x2=copy(x1)
# matlab/intersections.m:102
            y2=copy(y1)
# matlab/intersections.m:103
            self_intersect=copy(true)
# matlab/intersections.m:104
        else:
            if 4 == nargin:
                robust=copy(true)
# matlab/intersections.m:106
                self_intersect=copy(false)
# matlab/intersections.m:107
            else:
                if 5 == nargin:
                    self_intersect=copy(false)
# matlab/intersections.m:109
    ...
    x1=ravel(x1)
# matlab/intersections.m:125
    y1=ravel(y1)
# matlab/intersections.m:126
    x2=ravel(x2)
# matlab/intersections.m:127
    y2=ravel(y2)
# matlab/intersections.m:128
    # Compute number of line segments in each curve and some differences we'll
# need later.
    n1=length(x1) - 1
# matlab/intersections.m:132
    n2=length(x2) - 1
# matlab/intersections.m:133
    xy1=matlabarray(cat(x1,y1))
# matlab/intersections.m:134
    xy2=matlabarray(cat(x2,y2))
# matlab/intersections.m:135
    dxy1=diff(xy1)
# matlab/intersections.m:136
    dxy2=diff(xy2)
# matlab/intersections.m:137
...
# matlab/intersections.m:164
    else:
        remove=isnan(sum(dxy1[i,:] + dxy2[j,:],2))
# matlab/intersections.m:166
    
    i[remove]=[]
# matlab/intersections.m:168
    j[remove]=[]
# matlab/intersections.m:169
    # Initialize matrices.  We'll put the T's and B's in matrices and use them
# one column at a time.  AA is a 3-D extension of A where we'll use one
# plane at a time.
    n=length(i)
# matlab/intersections.m:174
    T=zeros(4,n)
# matlab/intersections.m:175
    AA=zeros(4,4,n)
# matlab/intersections.m:176
    AA[cat(1,2),3,:]=- 1
# matlab/intersections.m:177
    AA[cat(3,4),4,:]=- 1
# matlab/intersections.m:178
    AA[cat(1,3),1,:]=dxy1[i,:].T
# matlab/intersections.m:179
    AA[cat(2,4),2,:]=dxy2[j,:].T
# matlab/intersections.m:180
    B=- cat(x1[i],x2[j],y1[i],y2[j]).T
# matlab/intersections.m:181
...
# pairs.
    ...
    if robust:
        overlap=false(n,1)
# matlab/intersections.m:201
        warning_state=warning('off','MATLAB:singularMatrix')
# matlab/intersections.m:202
        try:
            lastwarn('')
            for k in arange(1,n).reshape(-1):
                T[:,k]=numpy.linalg.solve(AA[:,:,k],B[:,k])
# matlab/intersections.m:207
                unused,last_warn=lastwarn
# matlab/intersections.m:208
                lastwarn('')
                if strcmp(last_warn,'MATLAB:singularMatrix'):
                    # Force in_range(k) to be false.
                    T[1,k]=NaN
# matlab/intersections.m:212
                    overlap[k]=rcond(cat([dxy1[i[k],:]],[xy2[j[k],:] - xy1[i[k],:]])) < eps
# matlab/intersections.m:214
...
# matlab/intersections.m:224
        # intersection point that is at the center of the overlapping region.
        if any(overlap):
            ia=i[overlap]
# matlab/intersections.m:228
            ja=j[overlap]
# matlab/intersections.m:229
            T[3,overlap]=(max(min(x1[ia],x1[ia + 1]),min(x2[ja],x2[ja + 1])) + min(max(x1[ia],x1[ia + 1]),max(x2[ja],x2[ja + 1]))).T / 2
# matlab/intersections.m:231
            T[4,overlap]=(max(min(y1[ia],y1[ia + 1]),min(y2[ja],y2[ja + 1])) + min(max(y1[ia],y1[ia + 1]),max(y2[ja],y2[ja + 1]))).T / 2
# matlab/intersections.m:233
            selected=logical_or(in_range,overlap)
# matlab/intersections.m:235
        else:
            selected=copy(in_range)
# matlab/intersections.m:237
        xy0=T[3:4,selected].T
# matlab/intersections.m:239
        xy0,index=unique(xy0,'rows',nargout=2)
# matlab/intersections.m:242
        x0=xy0[:,1]
# matlab/intersections.m:243
        y0=xy0[:,2]
# matlab/intersections.m:244
        if nargout > 2:
            sel_index=find(selected)
# matlab/intersections.m:248
            sel=sel_index[index]
# matlab/intersections.m:249
            iout=i[sel] + T[1,sel].T
# matlab/intersections.m:250
            jout=j[sel] + T[2,sel].T
# matlab/intersections.m:251
    else:
        for k in arange(1,n).reshape(-1):
            L,U=lu(AA[:,:,k],nargout=2)
# matlab/intersections.m:255
            T[:,k]=numpy.linalg.solve(U,(numpy.linalg.solve(L,B[:,k])))
# matlab/intersections.m:256
        # Find where t1 and t2 are between 0 and 1 and return the corresponding
	# x0 and y0 values.
        in_range=(T[1,:] >= logical_and(0,T[2,:]) >= logical_and(0,T[1,:]) < logical_and(1,T[2,:]) < 1).T
# matlab/intersections.m:261
        x0=T[3,in_range].T
# matlab/intersections.m:262
        y0=T[4,in_range].T
# matlab/intersections.m:263
        if nargout > 2:
            iout=i[in_range] + T[1,in_range].T
# matlab/intersections.m:267
            jout=j[in_range] + T[2,in_range].T
# matlab/intersections.m:268
