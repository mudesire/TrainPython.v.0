...
# Trainmatlab.v.0/aps_era_ECMWF_Python.m

    
@function
def aps_era_ECMWF_Python(timing=None,wheatherregstr=None,*args,**kwargs):
    varargin = aps_era_ECMWF_Python.varargin
    nargin = aps_era_ECMWF_Python.nargin
...
# DB    03/2014 Auto download included
    ...
    year=timing[1:4]
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:30
    month=timing[5:6]
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:31
    day=timing[7:8]
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:32
    htime=timing[9:10]
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:33
    #mfield = calendar(str2double(year),str2double(month));
#nday = max(max(mfield));
    datestr=matlabarray(cat(year,'-',month,'-',day))
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:37
    pythonsc_path=matlabarray(cat(timing,'.py'))
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:39
    fid=fopen(pythonsc_path,'w')
# Trainmatlab.v.0/aps_era_ECMWF_Python.m:40
    ...
    fprintf(fid,cat('          \'target\'  : "ggap',num2str(timing[1:10]),'00.nc",\\n'))
    fprintf(fid,'                })\\n')