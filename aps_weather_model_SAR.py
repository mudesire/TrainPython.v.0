...
# Trainmatlab.v.0/aps_weather_model_SAR.m
...
# DB    07/2016     redefine hydrostatic delay to be based on surface pressure.
    ...
    fig_test=1
# Trainmatlab.v.0/aps_weather_model_SAR.m:71
    
    save_3D_delays=0
# Trainmatlab.v.0/aps_weather_model_SAR.m:72
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:78
    ## Constants
# Parameters from Jolivet et al 2011, GRL
    Rd=287.05
# Trainmatlab.v.0/aps_weather_model_SAR.m:82
    
    Rv=461.495
# Trainmatlab.v.0/aps_weather_model_SAR.m:83
    
    k1=0.776
# Trainmatlab.v.0/aps_weather_model_SAR.m:84
    
    k2=0.716
# Trainmatlab.v.0/aps_weather_model_SAR.m:85
    
    k3=3750.0
# Trainmatlab.v.0/aps_weather_model_SAR.m:86
    
    coeff.Rd = copy(Rd)
# Trainmatlab.v.0/aps_weather_model_SAR.m:87
    coeff.Rv = copy(Rv)
# Trainmatlab.v.0/aps_weather_model_SAR.m:88
    ## Defaults
    zref=15000
# Trainmatlab.v.0/aps_weather_model_SAR.m:91
    
    zincr=15
# Trainmatlab.v.0/aps_weather_model_SAR.m:92
    
    vertres=5
# Trainmatlab.v.0/aps_weather_model_SAR.m:93
    
    ## output file names
# output file for the DEM and look angles
    smpdem='dem_smp.xyz'
# Trainmatlab.v.0/aps_weather_model_SAR.m:97
    # getting the variables from the parms_aps file
    stamps_processed=getparm_aps('stamps_processed',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:100
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:103
    
    era_data_type=matlabarray([])
# Trainmatlab.v.0/aps_weather_model_SAR.m:104
    
    ### Updating specific weather model information
    if strcmpi(model_type,'era'):
        weather_model_datapath=getparm_aps('era_datapath',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:109
        era_data_type=getparm_aps('era_data_type')
# Trainmatlab.v.0/aps_weather_model_SAR.m:110
    else:
        if strcmpi(model_type,'merra') or strcmpi(model_type,'merra2'):
            weather_model_datapath=getparm_aps('merra_datapath',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:112
        else:
            error(cat('weather model type not supported, either: wrf, era, merra for now'))
    
    lambda_=dot(getparm_aps('lambda',1),100)
# Trainmatlab.v.0/aps_weather_model_SAR.m:117
    
    demfile=getparm_aps('demfile',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:118
    demnull=getparm_aps('dem_null',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:119
    UTC_sat=getparm_aps('UTC_sat',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:120
    ifgday_matfile=getparm_aps('ifgday_matfile',1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:121
    ifgs_dates=load(ifgday_matfile)
# Trainmatlab.v.0/aps_weather_model_SAR.m:122
    # loading the data
    if strcmp(stamps_processed,'y'):
        dates=ifgs_dates.day
# Trainmatlab.v.0/aps_weather_model_SAR.m:128
        load(psver)
        fprintf('Stamps processed structure \\n')
    else:
        psver=2
# Trainmatlab.v.0/aps_weather_model_SAR.m:132
        ifgs_dates=ifgs_dates.ifgday
# Trainmatlab.v.0/aps_weather_model_SAR.m:133
        dates=reshape(ifgs_dates,[],1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:134
        dates=unique(dates)
# Trainmatlab.v.0/aps_weather_model_SAR.m:135
        dates=datenum(num2str(dates),'yyyymmdd')
# Trainmatlab.v.0/aps_weather_model_SAR.m:136
    
    # getting the dates
    n_dates=length(dates)
# Trainmatlab.v.0/aps_weather_model_SAR.m:140
    ## Compute and resample DEM
    dem,xmin,xmax,ymin,ymax,smpres,nncols,nnrows=get_DEM
# Trainmatlab.v.0/aps_weather_model_SAR.m:143
...
# Should be  larger than the region to which the delay is computed
    lonmin=floor(xmin) - 1
# Trainmatlab.v.0/aps_weather_model_SAR.m:147
    lonmax=ceil(xmax) + 1
# Trainmatlab.v.0/aps_weather_model_SAR.m:148
    latmin=floor(ymin) - 1
# Trainmatlab.v.0/aps_weather_model_SAR.m:149
    latmax=ceil(ymax) + 1
# Trainmatlab.v.0/aps_weather_model_SAR.m:150
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:154
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:160
    ## generating a file
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:163
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:170
        hydroutfile=matlabarray(cat(weather_model_datapath,filesep,date_before[d,:],filesep,date_before[d,:],'_ZHD.xyz'))
# Trainmatlab.v.0/aps_weather_model_SAR.m:171
        no_data=0
# Trainmatlab.v.0/aps_weather_model_SAR.m:173
        for kk in arange(1,2).reshape(-1):
            if kk == 1:
                file=modelfile_before[d,:]
# Trainmatlab.v.0/aps_weather_model_SAR.m:176
            if kk == 2:
                file=modelfile_after[d,:]
# Trainmatlab.v.0/aps_weather_model_SAR.m:179
            # check
            if exist(file,'file') != 2:
                no_data=no_data + 1
# Trainmatlab.v.0/aps_weather_model_SAR.m:185
            else:
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:198
                latlist=reshape(latgrid[:,:,1],[],1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:202
                lonlist=reshape(longrid[:,:,1],[],1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:203
                xlist=reshape(xx,[],1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:204
                ylist=reshape(yy,[],1)
# Trainmatlab.v.0/aps_weather_model_SAR.m:205
                # Getting the weather model resolution
                lat_res=dot(abs(diff(unique(latgrid))),1.5)
# Trainmatlab.v.0/aps_weather_model_SAR.m:209
                lat_res=lat_res[1]
# Trainmatlab.v.0/aps_weather_model_SAR.m:210
                lon_res=dot(abs(diff(unique(longrid))),1.5)
# Trainmatlab.v.0/aps_weather_model_SAR.m:211
                lon_res=lon_res[1]
# Trainmatlab.v.0/aps_weather_model_SAR.m:212
            ...
            # degrees. User can be [-180 180] unlikely [0 360]
                if strcmpi(lon0360_flag,'y'):
                    ...
                    if xmin < 0:
                        xmin=xmin + 360
# Trainmatlab.v.0/aps_weather_model_SAR.m:223
                    if xmax < 0:
                        xmax=xmax + 360
# Trainmatlab.v.0/aps_weather_model_SAR.m:226
                # generation a plot
                if fig_test == logical_and(1,d) == logical_and(1,kk) == 1:
                    fontsize=15
# Trainmatlab.v.0/aps_weather_model_SAR.m:233
                    hfig=figure('name',cat(model_type,' weather model nodes and InSAR region'),'position',cat(200,243,610,603))
# Trainmatlab.v.0/aps_weather_model_SAR.m:234
                    ...
                    cc=copy(colorbar)
# Trainmatlab.v.0/aps_weather_model_SAR.m:243
                    view(0,90)
                    axis('xy')
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:264
                xlist=xlist[ix]
# Trainmatlab.v.0/aps_weather_model_SAR.m:265
                ylist=ylist[ix]
# Trainmatlab.v.0/aps_weather_model_SAR.m:266
                latlist=latlist[ix]
# Trainmatlab.v.0/aps_weather_model_SAR.m:267
                lonlist=lonlist[ix]
# Trainmatlab.v.0/aps_weather_model_SAR.m:268
                numy=length(unique(latlist))
# Trainmatlab.v.0/aps_weather_model_SAR.m:269
                numx=length(unique(lonlist))
# Trainmatlab.v.0/aps_weather_model_SAR.m:270
                ulatlist=unique(latlist)
# Trainmatlab.v.0/aps_weather_model_SAR.m:271
                ulonlist=unique(lonlist)
# Trainmatlab.v.0/aps_weather_model_SAR.m:272
                uxlist=unique(xlist)
# Trainmatlab.v.0/aps_weather_model_SAR.m:273
                uylist=unique(ylist)
# Trainmatlab.v.0/aps_weather_model_SAR.m:274
                if fig_test == logical_and(1,d) == logical_and(1,kk) == 1:
                    figure(hfig)
                    hold('on')
                    ...
                    str=''
# Trainmatlab.v.0/aps_weather_model_SAR.m:282
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:285
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:297
                deminfo.xmax = copy(xmax)
# Trainmatlab.v.0/aps_weather_model_SAR.m:298
                deminfo.ymax = copy(ymax)
# Trainmatlab.v.0/aps_weather_model_SAR.m:299
                deminfo.ymin = copy(ymin)
# Trainmatlab.v.0/aps_weather_model_SAR.m:300
                deminfo.dem = copy(dem)
# Trainmatlab.v.0/aps_weather_model_SAR.m:301
                ...
                g0=9.80665
# Trainmatlab.v.0/aps_weather_model_SAR.m:321
                H=Geopot / g0
# Trainmatlab.v.0/aps_weather_model_SAR.m:323
                g=multiply(9.80616,(1 - multiply(0.002637,cosd(dot(2.0,latgrid))) + multiply(5.9e-06,(cosd(dot(2.0,latgrid))) ** 2)))
# Trainmatlab.v.0/aps_weather_model_SAR.m:326
                Rmax=6378137
# Trainmatlab.v.0/aps_weather_model_SAR.m:328
                Rmin=6356752
# Trainmatlab.v.0/aps_weather_model_SAR.m:329
                Re=sqrt(1.0 / (((cosd(latgrid) ** 2) / Rmax ** 2) + ((sind(latgrid) ** 2) / Rmin ** 2)))
# Trainmatlab.v.0/aps_weather_model_SAR.m:330
                Z=(multiply(H,Re)) / (multiply(g / g0,Re) - H)
# Trainmatlab.v.0/aps_weather_model_SAR.m:333
                midx=round(mean(uxlist))
# Trainmatlab.v.0/aps_weather_model_SAR.m:336
                midy=round(mean(uylist))
# Trainmatlab.v.0/aps_weather_model_SAR.m:337
                glocal=g[midy,midx,1]
# Trainmatlab.v.0/aps_weather_model_SAR.m:338
                Rlocal=Re[midy,midx,1]
# Trainmatlab.v.0/aps_weather_model_SAR.m:339
                cdslices=maxdem / vertres + 1
# Trainmatlab.v.0/aps_weather_model_SAR.m:341
                cdstack=zeros(numy,numx,cdslices)
# Trainmatlab.v.0/aps_weather_model_SAR.m:342
                cdstack_dry=zeros(numy,numx,cdslices)
# Trainmatlab.v.0/aps_weather_model_SAR.m:343
                cdstack_wet=zeros(numy,numx,cdslices)
# Trainmatlab.v.0/aps_weather_model_SAR.m:344
                XI=(arange(0,zref,zincr)).T
# Trainmatlab.v.0/aps_weather_model_SAR.m:346
                gh=multiply(glocal,(Rlocal / (Rlocal + XI)) ** 2)
# Trainmatlab.v.0/aps_weather_model_SAR.m:347
            ...
            # then integrate using trapz to estimate delay as function of height
                for i in arange(1,numx).reshape(-1):
                    for j in arange(1,numy).reshape(-1):
                        xn=uxlist[i]
# Trainmatlab.v.0/aps_weather_model_SAR.m:353
                        yn=uylist[j]
# Trainmatlab.v.0/aps_weather_model_SAR.m:354
                        X=double(squeeze(Z[yn,xn,:]))
# Trainmatlab.v.0/aps_weather_model_SAR.m:357
                        Ye=double(squeeze(e[yn,xn,:]))
# Trainmatlab.v.0/aps_weather_model_SAR.m:358
                        YeI=dot(interp1(X,Ye,XI,'spline'),100)
# Trainmatlab.v.0/aps_weather_model_SAR.m:359
                        YP=double(squeeze(P[yn,xn,:]))
# Trainmatlab.v.0/aps_weather_model_SAR.m:361
                        YPI=dot(interp1(X,YP,XI,'spline'),100)
# Trainmatlab.v.0/aps_weather_model_SAR.m:362
                        YT=double(squeeze(Temp[yn,xn,:]))
# Trainmatlab.v.0/aps_weather_model_SAR.m:364
                        YTI=interp1(X,YT,XI,'spline')
# Trainmatlab.v.0/aps_weather_model_SAR.m:365
                        tmp1=(multiply((k2 - (dot(Rd,k1) / Rv)),YeI) / YTI + multiply(k3,YeI) / (YTI ** 2))
# Trainmatlab.v.0/aps_weather_model_SAR.m:367
                        Lw=dot(multiply((10 ** - 6),- 1),flipud(cumtrapz(flipud(XI),flipud(tmp1))))
# Trainmatlab.v.0/aps_weather_model_SAR.m:368
                        # tmp2 = k1.*YPI./YTI;
                    #Ld = (10^-6).*-1*flipud(cumtrapz(flipud(XI),flipud(tmp2)));             # This is using P/T expression (Hanssen, 2001)
                        gm=copy(glocal)
# Trainmatlab.v.0/aps_weather_model_SAR.m:372
                        Ld=multiply((10 ** - 6),(multiply((dot(k1,Rd) / gm),(YPI - YPI[zref / zincr + 1]))))
# Trainmatlab.v.0/aps_weather_model_SAR.m:373
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:378
                        LdI=interp1(XI,Ld,cdI,'spline')
# Trainmatlab.v.0/aps_weather_model_SAR.m:379
                        LwI=interp1(XI,Lw,cdI,'spline')
# Trainmatlab.v.0/aps_weather_model_SAR.m:380
                        cdstack_dry[j,i,:]=LdI
# Trainmatlab.v.0/aps_weather_model_SAR.m:383
                        cdstack_wet[j,i,:]=LwI
# Trainmatlab.v.0/aps_weather_model_SAR.m:384
                        if save_3D_delays == 1:
                            cdI3D=(arange(0,max(XI),100)).T
# Trainmatlab.v.0/aps_weather_model_SAR.m:387
                            LdI3D=interp1(XI,Ld,cdI3D,'spline')
# Trainmatlab.v.0/aps_weather_model_SAR.m:388
                            LwI3D=interp1(XI,Lw,cdI3D,'spline')
# Trainmatlab.v.0/aps_weather_model_SAR.m:389
                            cdstack_dry3D[j,i,:]=LdI3D
# Trainmatlab.v.0/aps_weather_model_SAR.m:392
                            cdstack_wet3D[j,i,:]=LwI3D
# Trainmatlab.v.0/aps_weather_model_SAR.m:393
                            clear('LdI3D','LwI3D')
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:403
                ysmpres=(ymax - ymin) / nnrows
# Trainmatlab.v.0/aps_weather_model_SAR.m:404
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:405
                ix_temp=diff(lonlist)
# Trainmatlab.v.0/aps_weather_model_SAR.m:407
                ix_temp=find(ix_temp != 0)
# Trainmatlab.v.0/aps_weather_model_SAR.m:408
                ix_temp=ix_temp[1]
# Trainmatlab.v.0/aps_weather_model_SAR.m:409
                lonlist_matrix=reshape(lonlist,ix_temp,[])
# Trainmatlab.v.0/aps_weather_model_SAR.m:410
                latlist_matrix=reshape(latlist,ix_temp,[])
# Trainmatlab.v.0/aps_weather_model_SAR.m:411
                ...
                if save_3D_delays == 1:
                    if kk == 1:
                        ...
                        hgt=copy(cdI3D)
# Trainmatlab.v.0/aps_weather_model_SAR.m:418
                        lon=matlabarray(cat(lonlist_matrix))
# Trainmatlab.v.0/aps_weather_model_SAR.m:419
                        lat=copy(latlist_matrix)
# Trainmatlab.v.0/aps_weather_model_SAR.m:420
                        dry1=copy(cdstack_dry3D)
# Trainmatlab.v.0/aps_weather_model_SAR.m:421
                        wet1=copy(cdstack_wet3D)
# Trainmatlab.v.0/aps_weather_model_SAR.m:422
                        dem_temp=copy(dem)
# Trainmatlab.v.0/aps_weather_model_SAR.m:428
                        dem_temp[isnan(dem_temp)]=0
# Trainmatlab.v.0/aps_weather_model_SAR.m:429
                        hgt_topo=griddata(xi,yi,dem_temp,lon,lat,'linear')
# Trainmatlab.v.0/aps_weather_model_SAR.m:430
                        clear('dem_temp')
                    else:
                        if kk == 2:
                            clear('dry2','wet2')
                            dry2=copy(cdstack_dry3D)
# Trainmatlab.v.0/aps_weather_model_SAR.m:436
                            wet2=copy(cdstack_wet3D)
# Trainmatlab.v.0/aps_weather_model_SAR.m:437
                clear('cdstack_interp_dry')
                cdstack_interp_dry=zeros(nnrows,nncols,cdslices)
# Trainmatlab.v.0/aps_weather_model_SAR.m:442
                for n in arange(1,cdslices).reshape(-1):
                    newslice=interp2(lonlist_matrix,latlist_matrix,cdstack_dry[:,:,n],xi,yi,'linear')
# Trainmatlab.v.0/aps_weather_model_SAR.m:444
                    cdstack_interp_dry[:,:,n]=flipud(newslice)
# Trainmatlab.v.0/aps_weather_model_SAR.m:445
                clear('cdstack_interp_wet')
                cdstack_interp_wet=zeros(nnrows,nncols,cdslices)
# Trainmatlab.v.0/aps_weather_model_SAR.m:449
                for n in arange(1,cdslices).reshape(-1):
                    newslice=interp2(lonlist_matrix,latlist_matrix,cdstack_wet[:,:,n],xi,yi,'linear')
# Trainmatlab.v.0/aps_weather_model_SAR.m:451
                    cdstack_interp_wet[:,:,n]=flipud(newslice)
# Trainmatlab.v.0/aps_weather_model_SAR.m:452
                clear('lonlist_matrix','latlist_matrix')
                # keeping the coordinates in the same grid as the data
                xi=flipud(xi)
# Trainmatlab.v.0/aps_weather_model_SAR.m:456
                yi=flipud(yi)
# Trainmatlab.v.0/aps_weather_model_SAR.m:457
                clear('wetcorrection','hydrcorrection','rounddem')
                wetcorrection=ones(nnrows,nncols)
# Trainmatlab.v.0/aps_weather_model_SAR.m:462
                hydrcorrection=ones(nnrows,nncols)
# Trainmatlab.v.0/aps_weather_model_SAR.m:463
                rounddem=round(dem / vertres)
# Trainmatlab.v.0/aps_weather_model_SAR.m:464
                rounddem[dem < 0]=0
# Trainmatlab.v.0/aps_weather_model_SAR.m:465
                rounddem[isnan(dem)]=0
# Trainmatlab.v.0/aps_weather_model_SAR.m:468
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:472
                for i in arange(1,nnrows).reshape(-1):
                    for j in arange(1,nncols).reshape(-1):
                        hydrcorrection[i,j]=cdstack_interp_dry[i,j,rounddem[i,j] + 1]
# Trainmatlab.v.0/aps_weather_model_SAR.m:478
                if kk == 1:
                    wetcorrection1=copy(wetcorrection)
# Trainmatlab.v.0/aps_weather_model_SAR.m:483
                    drycorrection1=copy(hydrcorrection)
# Trainmatlab.v.0/aps_weather_model_SAR.m:484
                if kk == 2:
                    wetcorrection2=copy(wetcorrection)
# Trainmatlab.v.0/aps_weather_model_SAR.m:487
                    drycorrection2=copy(hydrcorrection)
# Trainmatlab.v.0/aps_weather_model_SAR.m:488
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:542
            clear('wetcorrection1','wetcorrection2')
            wetcorrection=dot(wetcorrection,100)
# Trainmatlab.v.0/aps_weather_model_SAR.m:544
            fid=fopen(outfile,'w')
# Trainmatlab.v.0/aps_weather_model_SAR.m:545
            data_write=cat(reshape(xi,[],1),reshape(yi,[],1),reshape(wetcorrection,[],1)).T
# Trainmatlab.v.0/aps_weather_model_SAR.m:546
            tally=fwrite(fid,data_write,'double')
# Trainmatlab.v.0/aps_weather_model_SAR.m:547
            fclose(fid)
...
# Trainmatlab.v.0/aps_weather_model_SAR.m:552
            clear('drycorrection1','drycorrection2')
            hydrcorrection=dot(hydrcorrection,100)
# Trainmatlab.v.0/aps_weather_model_SAR.m:554
            fid=fopen(hydroutfile,'w')
# Trainmatlab.v.0/aps_weather_model_SAR.m:555
            data_write=cat(reshape(xi,[],1),reshape(yi,[],1),reshape(hydrcorrection,[],1)).T
# Trainmatlab.v.0/aps_weather_model_SAR.m:556
            tally=fwrite(fid,data_write,'double')
# Trainmatlab.v.0/aps_weather_model_SAR.m:557
            fclose(fid)
