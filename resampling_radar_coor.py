...
# matlab/resampling_radar_coor.m
...
#     51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA.
    ...
    if nargin < 1:
        multi_look_range=20
# matlab/resampling_radar_coor.m:30
    
    ar=ps.ar
# matlab/resampling_radar_coor.m:33
    ps.ij_new = copy(ps.ij)
# matlab/resampling_radar_coor.m:34
    ps.ij_new[:,3]=ceil(dot(ps.ij(arange(),3) / max(ps.ij(arange(),3)),multi_look_range))
# matlab/resampling_radar_coor.m:35
    ps.ij_new[:,2]=ceil(dot(ps.ij(arange(),2) / ar / max(ps.ij(arange(),3)),multi_look_range))
# matlab/resampling_radar_coor.m:36
    ij=NaN(cat(1,3))
# matlab/resampling_radar_coor.m:39
    xy=NaN(cat(1,3))
# matlab/resampling_radar_coor.m:40
    lonlat=NaN(cat(1,2))
# matlab/resampling_radar_coor.m:41
    phase=NaN(cat(1,size(phase_in,2)))
# matlab/resampling_radar_coor.m:42
    counter=1
# matlab/resampling_radar_coor.m:45
    n_grid=dot(max(ps.ij_new(arange(),2)),max(ps.ij_new(arange(),3)))
# matlab/resampling_radar_coor.m:46
    h=waitbar(0,'Resampling data, Please wait...')
# matlab/resampling_radar_coor.m:47
...
# matlab/resampling_radar_coor.m:51
            if isempty(ix) != 1:
                ij[counter,1]=counter
# matlab/resampling_radar_coor.m:53
                xy[counter,1]=counter
# matlab/resampling_radar_coor.m:54
                ij[counter,2:3]=cat(i,k)
# matlab/resampling_radar_coor.m:55
                xy[counter,2:3]=mean(ps.xy(ix,arange(2,3)),1)
# matlab/resampling_radar_coor.m:56
                lonlat[counter,1:2]=mean(ps.lonlat(ix,arange()),1)
# matlab/resampling_radar_coor.m:57
                A=ones(cat(size(ix,1),1))
# matlab/resampling_radar_coor.m:60
                phase[counter,:]=lscov(A,phase_in[ix,:])
# matlab/resampling_radar_coor.m:61
                counter=counter + 1
# matlab/resampling_radar_coor.m:64
                waitbar(counter / n_grid)
    
    close_(h)
    # reconstruct the matrix in radar coordiantes
    phase_matrix=NaN(cat(max(ij[:,2:3]),size(phase_in,2)))
# matlab/resampling_radar_coor.m:73
    for k in arange(1,ij[end(),1]).reshape(-1):
        phase_matrix[ij[k,2],ij[k,3],:]=phase[k,:]
# matlab/resampling_radar_coor.m:76
    
    bperp=ps.bperp
# matlab/resampling_radar_coor.m:80
    day=ps.day
# matlab/resampling_radar_coor.m:81
    day_ix=ps.day_ix
# matlab/resampling_radar_coor.m:82
    master_day=ps.master_day
# matlab/resampling_radar_coor.m:83
    master_ix=ps.master_ix
# matlab/resampling_radar_coor.m:84
    n_ifg=ps.n_ifg
# matlab/resampling_radar_coor.m:85
    n_image=ps.n_image
# matlab/resampling_radar_coor.m:86
    n_ps=size(lonlat,1)
# matlab/resampling_radar_coor.m:87