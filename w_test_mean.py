...
# PhD student - University of Leeds
    ...
    if size(y,2) > 1:
        y=y.T
# Trainmatlab.v.0/w_test_mean.m:48
        if size(y,2) > 1:
    ...
    if nargin < 2 or isempty(y_std):
        y_std=ones(size(y))
# Trainmatlab.v.0/w_test_mean.m:54
    
    if nargin < 3 or isempty(alpha):
        alpha=0.05
# Trainmatlab.v.0/w_test_mean.m:57
...
# Trainmatlab.v.0/w_test_mean.m:65
    # a variable to trace which observations were rejected
    obs_kept=arange(1,length(y))
# Trainmatlab.v.0/w_test_mean.m:70
    continue_reject=1
# Trainmatlab.v.0/w_test_mean.m:72
    while continue_reject:

        # Total number of observations to start with
        n_obs=length(y)
# Trainmatlab.v.0/w_test_mean.m:75
        Qyy=diag(y_std) ** 2
# Trainmatlab.v.0/w_test_mean.m:78
        A=ones(cat(n_obs,1))
# Trainmatlab.v.0/w_test_mean.m:81
        Qyy_inv=inv(Qyy)
# Trainmatlab.v.0/w_test_mean.m:84
        x_est=dot(dot(numpy.linalg.solve((dot(dot(A.T,Qyy_inv),A)),A.T),Qyy_inv),y)
# Trainmatlab.v.0/w_test_mean.m:87
        y_est=dot(A,x_est)
# Trainmatlab.v.0/w_test_mean.m:90
        e_est=y - y_est
# Trainmatlab.v.0/w_test_mean.m:91
        Qx_estx_est=inv(dot(dot(A.T,Qyy_inv),A))
# Trainmatlab.v.0/w_test_mean.m:94
        Qy_esty_est=dot(dot(A,Qx_estx_est),A.T)
# Trainmatlab.v.0/w_test_mean.m:95
        Qe_este_est=Qyy - Qy_esty_est
# Trainmatlab.v.0/w_test_mean.m:96
        # computation of the weights
        w=NaN(cat(n_obs,1))
# Trainmatlab.v.0/w_test_mean.m:99
        for k in arange(1,n_obs).reshape(-1):
            # setting the cy colum vector
            cy=zeros(cat(n_obs,1))
# Trainmatlab.v.0/w_test_mean.m:102
            cy[k]=1
# Trainmatlab.v.0/w_test_mean.m:103
            w[k]=(dot(dot(cy.T,Qyy_inv),e_est)) / (sqrt(dot(dot(dot(dot(cy.T,Qyy_inv),Qe_este_est),Qyy_inv),cy)))
# Trainmatlab.v.0/w_test_mean.m:106
            clear('cy')
        # take the absolute w-weight
        w=abs(w)
# Trainmatlab.v.0/w_test_mean.m:112
        ix=find(max(w) == w)
# Trainmatlab.v.0/w_test_mean.m:115
        if length(ix) > 1:
            ix=ix[1]
# Trainmatlab.v.0/w_test_mean.m:117
        ...
        # to be incorect
            if n_obs == 1:
                continue_reject=0
# Trainmatlab.v.0/w_test_mean.m:128
            else:
                # removing the identified observation as it appears and outlier
                y[ix]=[]
# Trainmatlab.v.0/w_test_mean.m:131
                y_std[ix,:]=[]
# Trainmatlab.v.0/w_test_mean.m:132
                obs_kept[ix]=[]
# Trainmatlab.v.0/w_test_mean.m:133
                clear('ix')
        else:
            clear('ix')
            # no more outliers found, end the outlier rejection
            continue_reject=0
# Trainmatlab.v.0/w_test_mean.m:139
