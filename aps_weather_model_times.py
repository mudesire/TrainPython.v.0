...
# Trainmatlab.v.0/aps_weather_model_times.m

    
@function
def aps_weather_model_times(timelist=None,dates=None,UTC_sat=None,*args,**kwargs):
    varargin = aps_weather_model_times.varargin
    nargin = aps_weather_model_times.nargin
...
# Trainmatlab.v.0/aps_weather_model_times.m:16
    
    # number of SAR acquisitions
    n_SAR=length(dates)
# Trainmatlab.v.0/aps_weather_model_times.m:19
    # find two closest times with respect the the 6 hr ERA-I data
    timelist=matlabarray(cat(['0000'],['0600'],['1200'],['1800'],['0000']))
# Trainmatlab.v.0/aps_weather_model_times.m:22
    time=str2num(UTC_sat[1:2]) + str2num(UTC_sat[4:5]) / 60
# Trainmatlab.v.0/aps_weather_model_times.m:23
    t_before=floor(time / 6)
# Trainmatlab.v.0/aps_weather_model_times.m:24
    t_after=ceil(time / 6)
# Trainmatlab.v.0/aps_weather_model_times.m:25
...
# Trainmatlab.v.0/aps_weather_model_times.m:29
    f_after[isnan(f_after)]=1
# Trainmatlab.v.0/aps_weather_model_times.m:30
    f_before=1 - f_after
# Trainmatlab.v.0/aps_weather_model_times.m:31
    f_after=repmat(f_after,n_SAR,1)
# Trainmatlab.v.0/aps_weather_model_times.m:32
    f_before=repmat(f_before,n_SAR,1)
# Trainmatlab.v.0/aps_weather_model_times.m:33
    # the time stamp of the closest two ERA acquisitions
    time_before=repmat(num2str(timelist[t_before + 1,:]),n_SAR,1)
# Trainmatlab.v.0/aps_weather_model_times.m:36
    time_after=repmat(num2str(timelist[t_after + 1,:]),n_SAR,1)
# Trainmatlab.v.0/aps_weather_model_times.m:37
...
# Trainmatlab.v.0/aps_weather_model_times.m:44
    if t_after == 4:
        date_after=datestr(dates + 1,datestructure)
# Trainmatlab.v.0/aps_weather_model_times.m:46
    else:
        date_after=datestr(dates,datestructure)
# Trainmatlab.v.0/aps_weather_model_times.m:48
