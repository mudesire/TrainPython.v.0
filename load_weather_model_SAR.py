...
# Trainmatlab.v.0/load_weather_model_SAR.m
...
# DB    29/04/2016      Include a fix in case the coordiantes are way off during geocooding.
    ...
    plot_flag=0
# Trainmatlab.v.0/load_weather_model_SAR.m:35
...
# all data at once.
    chunk_flag='y'
# Trainmatlab.v.0/load_weather_model_SAR.m:51
    
    chunk_bin_size=0.5
# Trainmatlab.v.0/load_weather_model_SAR.m:52
    ...
    if nargin < 2 or isempty(xy_out_grid):
        interpolate_flag=0
# Trainmatlab.v.0/load_weather_model_SAR.m:58
    else:
        interpolate_flag=1
# Trainmatlab.v.0/load_weather_model_SAR.m:60
    
    if nargin < 3 or isempty(overwrite_flag):
        overwrite_flag=1
# Trainmatlab.v.0/load_weather_model_SAR.m:63
    
    
    ## checking the file type if its grd or not
    file_type_grd='n'
# Trainmatlab.v.0/load_weather_model_SAR.m:70
    file_path,temp,file_ext=fileparts(filename,nargout=3)
# Trainmatlab.v.0/load_weather_model_SAR.m:71
    # check if this is a grd file already
    if strcmpi(file_ext,'.grd'):
        fprintf(cat('Specified aps correction is a .grd file\\n'))
        file_type_grd='y'
# Trainmatlab.v.0/load_weather_model_SAR.m:75
        xyz_input=copy(inf)
# Trainmatlab.v.0/load_weather_model_SAR.m:76
        gmt5_above,gmt_version=get_gmt_version(nargout=2)
# Trainmatlab.v.0/load_weather_model_SAR.m:78
        error('This is currently under development')
    else:
        ## Loading of processed data files
        fid=fopen(filename,'r')
# Trainmatlab.v.0/load_weather_model_SAR.m:83
        data_vector=fread(fid,'double')
# Trainmatlab.v.0/load_weather_model_SAR.m:84
        fclose(fid)
        xyz_input=reshape(data_vector,3,[]).T
# Trainmatlab.v.0/load_weather_model_SAR.m:88
        clear('data_vector')
...
# overwrite any specific crop file then.
    if logical_not(strcmpi(file_ext,'.grd')):
        if interpolate_flag == 1:
            crop_flag=1
# Trainmatlab.v.0/load_weather_model_SAR.m:99
            xy_min=min(xy_out_grid)
# Trainmatlab.v.0/load_weather_model_SAR.m:100
            xy_max=max(xy_out_grid)
# Trainmatlab.v.0/load_weather_model_SAR.m:101
            xy_min=xy_min - 0.01
# Trainmatlab.v.0/load_weather_model_SAR.m:102
            xy_max=xy_max + 0.01
# Trainmatlab.v.0/load_weather_model_SAR.m:103
            poly.xy = copy(cat([xy_min[1],xy_min[2]],[xy_min[1],xy_max[2]],[xy_max[1],xy_max[2]],[xy_max[1],xy_min[2]]))
# Trainmatlab.v.0/load_weather_model_SAR.m:105
            if xy_min[1] < logical_and(0,min(xyz_input[:,1])) > 0:
                xyz_input[:,1]=xyz_input[:,1] - 360
# Trainmatlab.v.0/load_weather_model_SAR.m:111
            # do the actual cropping
            ix=inpolygon(xyz_input[:,1],xyz_input[:,2],poly.xy(arange(),1),poly.xy(arange(),2))
# Trainmatlab.v.0/load_weather_model_SAR.m:116
            ix[ix > 1]=1
# Trainmatlab.v.0/load_weather_model_SAR.m:118
            xyz_output=xyz_input[ix,:]
# Trainmatlab.v.0/load_weather_model_SAR.m:121
            clear('ix')
...
# Trainmatlab.v.0/load_weather_model_SAR.m:134
            generate_file='n'
# Trainmatlab.v.0/load_weather_model_SAR.m:138
            if overwrite_flag == 0 and exist(lonlat_file,'file') != 2:
                generate_file='y'
# Trainmatlab.v.0/load_weather_model_SAR.m:140
            if overwrite_flag == 1:
                generate_file='y'
# Trainmatlab.v.0/load_weather_model_SAR.m:143
...
# Trainmatlab.v.0/load_weather_model_SAR.m:201
                    if sum(ix_temp) > 0:
...
# Trainmatlab.v.0/load_weather_model_SAR.m:203
