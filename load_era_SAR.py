...
# Trainmatlab.v.0/load_era_SAR.m

    
@function
def load_era_SAR(filename=None,xy_out_grid=None,overwrite_flag=None,*args,**kwargs):
    varargin = load_era_SAR.varargin
    nargin = load_era_SAR.nargin
...
#                       correct compiling of the mex code.
    ...
    plot_flag=0
# Trainmatlab.v.0/load_era_SAR.m:33
...
# all data at once.
    chunk_flag='y'
# Trainmatlab.v.0/load_era_SAR.m:49
    
    chunk_bin_size=0.5
# Trainmatlab.v.0/load_era_SAR.m:50
...
# overwrite any specific crop file then.
    if logical_not(strcmpi(file_ext,'.grd')):
        if interpolate_flag == 1:
            crop_flag=1
# Trainmatlab.v.0/load_era_SAR.m:97
            xy_min=min(xy_out_grid)
# Trainmatlab.v.0/load_era_SAR.m:98
            xy_max=max(xy_out_grid)
# Trainmatlab.v.0/load_era_SAR.m:99
            xy_min=xy_min - 0.01
# Trainmatlab.v.0/load_era_SAR.m:100
            xy_max=xy_max + 0.01
# Trainmatlab.v.0/load_era_SAR.m:101
            poly.xy = copy(cat([xy_min[1],xy_min[2]],[xy_min[1],xy_max[2]],[xy_max[1],xy_max[2]],[xy_max[1],xy_min[2]]))
# Trainmatlab.v.0/load_era_SAR.m:103
            if xy_min[1] < logical_and(0,min(xyz_input[:,1])) > 0:
                xyz_input[:,1]=xyz_input[:,1] - 360
# Trainmatlab.v.0/load_era_SAR.m:109
            # do the actual cropping
            ix=inpolygon(xyz_input[:,1],xyz_input[:,2],poly.xy(arange(),1),poly.xy(arange(),2))
# Trainmatlab.v.0/load_era_SAR.m:114
            ix[ix > 1]=1
# Trainmatlab.v.0/load_era_SAR.m:116
            xyz_output=xyz_input[ix,:]
# Trainmatlab.v.0/load_era_SAR.m:119
            clear('ix')
...
# Trainmatlab.v.0/load_era_SAR.m:132
            generate_file='n'
# Trainmatlab.v.0/load_era_SAR.m:136
            if overwrite_flag == 0 and exist(lonlat_file,'file') != 2:
                generate_file='y'
# Trainmatlab.v.0/load_era_SAR.m:138
            if overwrite_flag == 1:
                generate_file='y'
# Trainmatlab.v.0/load_era_SAR.m:141
...
# Trainmatlab.v.0/load_era_SAR.m:199
                    if sum(ix_temp) > 0:
...
# Trainmatlab.v.0/load_era_SAR.m:201
