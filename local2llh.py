...
#   Sep 7, 2000   Peter Cervelli		Original Code
    ...
    a=6378137.0
# Trainmatlab.v.0/local2llh.m:46
    e=0.0820944379497
# Trainmatlab.v.0/local2llh.m:47
    #Convert to radians / meters
    
    #   xy=xy*1000;
#   origin=origin*pi/180;
    xy=dot(double(xy),1000)
# Trainmatlab.v.0/local2llh.m:53
    origin=dot(double(origin),pi) / 180
# Trainmatlab.v.0/local2llh.m:54
...
# Trainmatlab.v.0/local2llh.m:58
    z=xy[2,:] != - M0
# Trainmatlab.v.0/local2llh.m:63
    A=(M0 + xy[2,z]) / a
# Trainmatlab.v.0/local2llh.m:65
    B=xy[1,z] ** 2.0 / a ** 2 + A ** 2
# Trainmatlab.v.0/local2llh.m:66
    llh[2,z]=A
# Trainmatlab.v.0/local2llh.m:68
    delta=copy(Inf)
# Trainmatlab.v.0/local2llh.m:70
    c=0
# Trainmatlab.v.0/local2llh.m:72
    while max(abs(delta)) > 1e-08:

        C=multiply(sqrt((1 - dot(e ** 2,sin(llh[2,z]) ** 2))),tan(llh[2,z]))
# Trainmatlab.v.0/local2llh.m:76
...
# Trainmatlab.v.0/local2llh.m:78
...
# Trainmatlab.v.0/local2llh.m:83
        Ma=M / a
# Trainmatlab.v.0/local2llh.m:88
...
# Trainmatlab.v.0/local2llh.m:90
        llh[2,z]=llh[2,z] + delta
# Trainmatlab.v.0/local2llh.m:93
        c=c + 1
# Trainmatlab.v.0/local2llh.m:95
        if c > 100:
            error('Convergence failure.')

    
    llh[1,z]=(asin(multiply(xy[1,z],C) / a)) / sin(llh[2,z]) + origin[1]
# Trainmatlab.v.0/local2llh.m:101
    #Handle special case of latitude = 0
    
    llh[1,logical_not(z)]=xy[1,logical_not(z)] / a + origin[1]
# Trainmatlab.v.0/local2llh.m:105
    llh[2,logical_not(z)]=0
# Trainmatlab.v.0/local2llh.m:106
    #Convert back to decimal degrees
    
    llh=dot(llh,180) / pi
# Trainmatlab.v.0/local2llh.m:110